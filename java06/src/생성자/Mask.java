...
public class Mask {
	...
	int price; //0, 기본형변수
	int count; //0, 기본형변수
	//멤버변수는 전역변수이다. 전역변수는 자동초기화가 된다.
	//멤버변수의 원본은 클래스원본영역에 저장이 된다.
	//멤버변수는 객체생성될 때, 힙영역에 복사가 된다.
	
	//객체생성후, 멤버변수값을 나중에 넣어도 상관없는 경우, 생성자 만들어주지 않아도 된다.
	
	//생성자가 하나도 없는 경우, 자동으로 입력값 없는 생성자를 만들어준다.
	//묵시적으로 생성자가 만들어진다.
...
//	}
	...
	//계좌처럼 파라메터없이 객체생성하고 싶고, 파라메터있는 객체생성하고 싶은 경우, 기본생성자를 명시적으로 써주어야 한다.
